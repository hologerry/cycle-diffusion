...
class DDPMDDIMWrapper(torch.nn.Module):
    ...
    def generate(self, z, class_label):
        ...
        if self.enforce_class_input:
            ...
            for it, (i, j) in tqdm(
                enumerate(zip(reversed(seq_inv), reversed(seq_inv_next))), total=len(seq_inv), desc="generating"
            ):
            ...
            if self.refine_steps == 0:
                ...
                for r in trange(self.refine_iterations, desc="refining out iter"):
                    refine_eta = 1
                    # Sample xt
                    ...
                    for i, j in tqdm(zip(reversed(seq_inv_refine), reversed(seq_inv_next_refine)), total=len(seq_inv_refine), desc="refining"):
                        t = (torch.ones(bsz) * i).to(self.device)
                        t_next = (torch.ones(bsz) * j).to(self.device)
                        x = denoising_step(
                            x,
    ...
    def encode(self, image, class_label=None):
        ...
        with torch.no_grad():
            ...
            if self.enforce_class_input:
                ...
                xt = xT
                for it, (i, j) in tqdm(
                    enumerate(zip(reversed(seq_inv), reversed(seq_inv_next))), total=len(seq_inv), desc="encoding"
                ):
                    t = (torch.ones(bsz) * i).to(self.device)
                    t_next = (torch.ones(bsz) * j).to(self.device)
