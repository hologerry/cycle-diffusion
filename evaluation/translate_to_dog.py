...
class Evaluator(object):
    def __init__(self, args, meta_args):
        ...
        self.ref_transform = transforms.Compose(
            [transforms.Resize(256, interpolation=INTERPOLATION), transforms.ToTensor()]  # 512 -> 256
        )
    ...
    def evaluate(self, images, model, weighted_loss, losses, data, split):
        ...
        ref_images = list_image_files_recursively(root_dir)
        for idx, file_name in tqdm(enumerate(ref_images), total=len(ref_images), desc="evaluating prepare reference images"):
        ...
        idx = 0
        for original_img, img in tqdm(images, desc="evaluating images"):
